...
class JobPostForm(forms.ModelForm):
    ...
    # Platforms
    instagram = forms.BooleanField(label='Instagram', initial=False, required=False, widget=forms.CheckboxInput(attrs={'class': 'form-control', 'onclick' : 'checkPlatformInstagram(this);getPriceTag();'}))
    facebook = forms.BooleanField(label='Facebook', initial=False, required=False, widget=forms.CheckboxInput(attrs={'class': 'form-control', 'onclick' : 'checkPlatformFacebook(this);getPriceTag();'}))

    # Platform Usernames
    instagram_username = forms.CharField(label='Instagram Username', required=False, widget=forms.TextInput(attrs={'class': 'form-control', 'readonly':'readonly'}))
    facebook_username = forms.CharField(label='Facebook Username', required=False, widget=forms.TextInput(attrs={ 'class': 'form-control', 'readonly':'readonly'}))
    
    # Services provided
    captions = forms.BooleanField(label='Create Captions', initial=False, required=False, widget=forms.CheckboxInput(attrs={'class': 'form-control', 'onclick' : 'getPriceTag();'}))
    ...
    class Meta:
        model = JobPost
        fields = [ "length", "number_of_post", "post_for_you", "engagement", "captions", "search_for_content", "service_description", "instagram_username", "facebook_username", "instagram", "facebook"]
...
class JobPostFormUpdate(forms.ModelForm):
    ...
    post_for_you = forms.BooleanField(label='Manager Will Post For You', initial=True, required=False)
    service_description = forms.CharField(label='Service Description',widget=forms.Textarea(attrs={'placeholder':'Enter ...', 'rows' : '5', 'class' : 'form-control' }))
    class Meta:
        model = JobPost
        fields = [ "length", "number_of_post", "post_for_you", "engagement", "captions", "search_for_content", "service_description"]